<template>
  <div class="product-card-container" v-on:mouseover="buttonVisible=true" v-on:mouseleave="buttonVisible=false">
    ...
    <img class="product-card-image" alt="product" :src="this.item.imagesList[0]"/>
    ...
    <div class="product-card-colors-block">
      <div class="product-card-color" v-for="(color, index) in this.item.colorsList" :key="index" :title="color.title" v-bind:style="{backgroundImage: `url(${color.thumbnail})`}"/>
    </div>
